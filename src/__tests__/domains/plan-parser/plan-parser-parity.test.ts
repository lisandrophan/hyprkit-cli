...
// Conditional require — only load if file exists (not available in CI)
const cjsParsePlanPhases = HAS_CJS_PARSER
	? (require(CJS_PARSER_PATH) as any).parsePlanPhases
	: () => [];
...
// All parity tests require the engineer CJS parser to be present (not available in CI)
describe.skipIf(!HAS_CJS_PARSER)("Parser Parity", () => {
...
}); // end Parser Parity skipIf wrapper
